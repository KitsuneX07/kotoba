...
async fn openai_responses_basic_text_dialog_live() {
    ...
    let response = match provider.chat(request).await {
        ...
        Err(LLMError::Auth { message }) => {
            eprintln!(
                "skip openai_responses_basic_text_dialog_live: auth error: {message}"
            );
        ...
        Err(LLMError::Transport { message }) => {
            eprintln!(
                "skip openai_responses_basic_text_dialog_live: transport error: {message}"
            );
            return;
        }
...
async fn openai_responses_basic_tool_call_dialog_live() {
    ...
    let response = match provider.chat(request).await {
        ...
        Err(LLMError::Auth { message }) => {
            eprintln!(
                "skip openai_responses_basic_tool_call_dialog_live: auth error: {message}"
            );
            return;
        }
        Err(LLMError::RateLimit { message, .. }) => {
            eprintln!(
                "skip openai_responses_basic_tool_call_dialog_live: rate limit: {message}"
            );
            return;
        }
        Err(LLMError::Transport { message }) => {
...
async fn openai_responses_live_sync_and_stream() {
    ...
    let response = match provider.chat(request.clone()).await {
        ...
        Err(LLMError::Auth { message }) => {
            eprintln!(
                "skip openai_responses_live_sync_and_stream (sync): auth error: {message}"
            );
            return;
        }
        Err(LLMError::RateLimit { message, .. }) => {
            eprintln!(
                "skip openai_responses_live_sync_and_stream (sync): rate limit: {message}"
            );
            return;
        }
        Err(LLMError::Transport { message }) => {
    ...
    let mut stream = match provider.stream_chat(request).await {
        ...
        Err(LLMError::Auth { message }) => {
            eprintln!(
                "skip openai_responses_live_sync_and_stream (stream): auth error: {message}"
            );
            return;
        }
        Err(LLMError::RateLimit { message, .. }) => {
            eprintln!(
                "skip openai_responses_live_sync_and_stream (stream): rate limit: {message}"
            );
            return;
        }
        Err(LLMError::Transport { message }) => {
